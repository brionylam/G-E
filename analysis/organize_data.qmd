...
variables_for_pca <- c("fund_research", "reg_CO2_pollutant", "discuss_GW", 
                       ...
                       "harm_US", "harm_future_gen", "harm_dev_countries")
...
# Get explained variances for Dim1 and Dim2
explained_variances <- pca_result$eig[1:2, "eigenvalue"]
explained_variance_perc <- explained_variances / sum(explained_variances)

# Ensure that the length of explained_variance_perc matches the number of variables
num_variables <- nrow(pca_result$var$coord)
explained_variance_perc <- rep(explained_variance_perc, num_variables)

# Create a data frame for biplot data
...
# Plot biplot using ggplot2
biplot <- ggplot(biplot_data, aes(x = Dim.1, y = Dim.2)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "gray") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray") +
  geom_text(aes(label = rownames(biplot_data)), size = 3, hjust = 0, vjust = 0) +
  geom_segment(aes(x = 0, y = 0, xend = Dim.1, yend = Dim.2), 
               arrow = arrow(length = unit(0.15, "inches")), color = "blue") +
  labs(x = paste("Dim.1 (", round(explained_variance_perc[1] * 100, 2), "%)"),
       y = paste("Dim.2 (", round(explained_variance_perc[2] * 100, 2), "%)")) +
  theme_minimal()
...
#Import data 
dv <- subset(dts, 
              select=c("fund_research","reg_CO2_pollutant","discuss_GW", "worry","harm_personally","harm_plants_animals","harm_future_gen","harm_US","harm_dev_countries"))

#"reg_CO2_pollutant" not asked in wave4, "harm_plants_animals" not asked in wave13
dv <- subset(dv, wave != 4 & wave != 13)

# in this dateset, "-1" menas reject to respond, drop all the "-1"


dv <- subset(dv, worry !=-1 & fund_research != -1 & reg_CO2_pollutant != -1 & discuss_GW != -1 & harm_personally != -1 & harm_plants_animals != -1 & harm_future_gen != -1 & harm_US != -1 & harm_dev_countries != -1 )
