...
ccam_imputed <- kNN(ccam[, variables_to_impute], variable = variables_to_impute, k = 5)

# Create indicator variables for imputed values
for (variable in variables_to_impute) {
  ccam[paste0("imputed_", variable)] <- ifelse(is.na(ccam_imputed[[variable]]), 1, 0)
  # Optional: Create indicator variable for original missing values (coded as 0)
  ccam[paste0("original_missing_", variable)] <- ifelse(ccam[[variable]] == 0, 1, 0)
}

# Update the dataset with imputed values by replacing only the imputed columns
ccam[ , variables_to_impute] <- ccam_imputed[, variables_to_impute]
